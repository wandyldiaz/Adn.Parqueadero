...
public abstract class AbstractParkingDatabase extends RoomDatabase {
    private  static final String DB_NAME="parking_db";
    private static AbstractParkingDatabase instance;

    public static  synchronized AbstractParkingDatabase getInstance(Context context){
        if(instance==null){
            try {
                instance = Room.databaseBuilder(context, AbstractParkingDatabase.class, DB_NAME).fallbackToDestructiveMigration()
                        .build();
            }catch(Exception e){
